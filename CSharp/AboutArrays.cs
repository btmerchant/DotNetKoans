...
namespace DotNetKoans.CSharp
{
    public class AboutArrays : Koan
    {
        ...
        {
            var empty_array = new object[] { };  //Makes an array of objects
        ...
        public void ArrayLiterals()
        {
            ...
            Assert.Equal(42, array[((int)'A' - 65)]); //should be 0 it is an array of objects (int)0 casts 0 as an int

            //This is important because...
            Assert.True(array.IsFixedSize);

            //...it means we can't do this: array[1] = 13;
            Assert.Throws(typeof(IndexOutOfRangeException), delegate() { array[1] = 13; }); //delegate creates an anonymous function as needed
            ...
            dynamicArray.Add(42);
            Assert.Equal(array, dynamicArray.ToArray()); //ToArray converts dynamic array to a normal array

            dynamicArray.Add(13);
            Assert.Equal((new int[] { 42, (int)'A' - 52}), dynamicArray.ToArray()); //should be 13
        }

        [Koan(3)]
        public void AccessingArrayElements()
        {
            var array = new[] { "peanut", "butter", "and", "jelly" }; //var array = new[] example of implied type
        ...
        [Koan(6)]
        public void Shifting()  //use new string
        {
